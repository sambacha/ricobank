...
contract Ball is Math, Ward {
    ...
    function setup(BallArgs calldata args) external _ward_ {
        ...
        _configureBlock(multiplier, RICO_USD_TAG, address(cladapt), DAI_USD_TAG, address(uniadapt), RICO_DAI_TAG, RAY);
        _configureBlock(divider, RICO_REF_TAG, address(multiplier), RICO_USD_TAG, address(cladapt), XAU_USD_TAG, RAY);
    ...
    function makeilk(IlkParams calldata ilkparams) external _ward_ {
        ...
        if (ilkparams.gemethagg == address(0)) {
            ...
            _configureBlock(multiplier, gemusdtag, address(cladapt), gemethtag, address(cladapt), WETH_USD_TAG, RAY);
    ...
    function makeuni(UniParams calldata ups) external _ward_ {
        ...
        Vat(bank).filh(ups.ilk, "pep", empty, bytes32(uint256(2)));
        Vat(bank).filh(ups.ilk, "pop", empty, bytes32(RAY));

        for (uint256 i = 0; i < ups.gems.length; i++) {
    ...
    function _configureBlock(Block b, bytes32 tag, address s1, bytes32 t1, address s2, bytes32 t2, uint256 C)
        internal
    {
        ...
        if (C == RAY) {
            ...
            tags = new bytes32[](3);
            sources[2] = address(this);
            tags[2] = bytes32(C);
            Feedbase(feedbase).push(bytes32(C), bytes32(C), type(uint256).max);
        }
    ...
    function read(bytes32 tag) external pure returns (bytes32, uint256) {
        return (tag, type(uint256).max);
    }
}
