...
interface IUniWrapper {
    function total(INFPM nfpm, uint256 tokenId, uint160 sqrtPriceX96)
        external
        view
...
contract UniNFTHook is HookMix {
    ...
    struct UniNFTHookStorage {
        ...
        mapping(address usr => uint256[] tokenIds) inks;
    ...
    function frobhook(FHParams calldata p) external payable returns (bool safer) {
        ...
        uint256[] storage tokenIds = hs.inks[p.u];
        ...
        int256 dir = LOCK;
        if (p.dink.length > 0) {
            dink = abi.decode(p.dink, (uint256[]));
            dir = int256(dink[0]);
        }
        ...
        if (dir == LOCK) {
            ...
            unchecked {
                // add uni positions
                for (uint256 idx = 1; idx < dink.length; idx++) {
                    // pull the token
                    uint256 tokenId = dink[idx];
            ...
            unchecked {
                ...
                for (uint256 j = 1; j < dink.length; j++) {
                    uint256 toss = dink[j];
    ...
    function bailhook(BHParams calldata p) external payable returns (bytes memory) {
        UniNFTHookStorage storage hs = getStorage(p.i);
        uint256[] memory ids = hs.inks[p.u];
        ...
        unchecked {
            uint256 over = earn > p.bill ? earn - p.bill : 0;
    ...
    function safehook(bytes32 i, address u) external view returns (uint256 tot, uint256 cut, uint256 minttl) {
        ...
        for (uint256 idx = 0; idx < pos.len;) {
            ...
            uint256 p1;
            {
                ...
                (bytes32 val0, uint256 ttl) = fb.pull(src0.rudd.src, src0.rudd.tag);
                p0 = uint256(val0) / BLN;
    ...
    function file(bytes32 key, bytes32 i, bytes32[] calldata xs, bytes32 val) external payable {
        ...
        if (xs.length == 0) {
            if (key == "room") hs.room = uint256(val);
            else if (key == "wrap") hs.wrap = IUniWrapper(address(bytes20(val)));
            else if (key == "pep") hs.plot.pep = uint256(val);
            else if (key == "pop") hs.plot.pop = uint256(val);
            else if (key == "pup") hs.plot.pup = int256(uint256(val));
            else revert ErrWrongKey();
            emit NewPalm1(key, i, val);
        } else if (xs.length == 1) {
            address gem = address(bytes20(xs[0]));
            if (key == "src") {
                hs.sources[gem].rudd.src = address(bytes20(val));
            } else if (key == "tag") {
                hs.sources[gem].rudd.tag = val;
            } else if (key == "liqr") {
                must(uint256(val), RAY, type(uint256).max);
                hs.sources[gem].liqr = uint256(val);
            } else {
                revert ErrWrongKey();
            }
