...
contract Vow is Bank {
    function RISK() external view returns (Gem) {
        return getVowStorage().risk;
    }
    ...
    function loot() external view returns (uint256) {
        return getVowStorage().loot;
    }

    function rudd() external view returns (Rudd memory) {
        return getVowStorage().rudd;
    }

    function plat() external view returns (Plx memory) {
        return getVowStorage().plat;
    }

    function plot() external view returns (Plx memory) {
        return getVowStorage().plot;
    }
    ...
    function keep(bytes32[] calldata ilks) external payable _flog_ {
        ...
        Gem rico = bankS.rico;
        Gem risk = vowS.risk;
        uint256 joy = vatS.joy;

        // use equal scales for sin and joy
        uint256 sin = vatS.sin / RAY;
        if (joy > sin) {
            ...
            uint256 mcap = rmul(price, risk.totalSupply());
            uint256 mash = rdiv(mcap, mcap + joy);
            mash = rmash(mash, vowS.plat.pep, vowS.plat.pop, vowS.plat.pup);

            // buy-and-burn risk with remaining (`flap`) rico
            uint256 flap = rmul(joy - 1, vowS.ramp.wel);
            joy -= flap;
            vatS.joy = joy;
            emit NewPalm0("joy", bytes32(joy));

            uint256 sell = rmul(flap, vowS.loot);
            uint256 earn = rmul(sell, rdiv(mash, price));
            ...
            uint256 price = _price();
            uint256 mcap = rmul(price, risk.totalSupply());
            uint256 mash = rdiv(mcap, mcap + under);
            mash = rmash(mash, vowS.plot.pep, vowS.plot.pop, vowS.plot.pup);

            // rate-limit flop
            uint256 elapsed = min(block.timestamp - vowS.ramp.bel, vowS.ramp.cel);
            uint256 flop = elapsed * rmul(vowS.ramp.rel, risk.totalSupply());
            if (0 == flop) revert ErrReflop();

            // swap RISK for rico to cover sin
            uint256 earn = rmul(flop, rmul(price, mash));
            uint256 bel = block.timestamp;
            if (earn > under) {
    ...
    function _heal(uint256 wad) internal returns (uint256 joy) {
    ...
    function _price() internal view returns (uint256) {
        BankStorage storage bankS = getBankStorage();
        VowStorage storage vowS = getVowStorage();
        (bytes32 _val, uint256 ttl) = bankS.fb.pull(vowS.rudd.src, vowS.rudd.tag);
