...
contract VatTest is Test, RicoSetUp {
    uint256 constant init_join = 1000;
    uint256 constant flash_size = 100;
    uint256 constant stack = WAD * 10;
    ...
    function test_frob_basic() public {
        feedpush(grtag, bytes32(1000 * RAY), type(uint256).max);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD));
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD));
    }
    ...
    function test_drip_basic() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(100 * WAD), int256(50 * WAD));
        Vat(bank).drip(gilk);
    }
    ...
    function test_create_unsafe() public {
        ...
        Vat(bank).frob(gilk, address(this), abi.encodePacked(stack), int256(stack) + 1);

        // skip past feed expiration
        (, uint256 ttl) = feedpull(grtag);
        skip(ttl - block.timestamp + 100);

        // shouldn't be able to frob to less safe position if feed is iffy
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, address(this), abi.encodePacked(stack), int256(1));
    }

    function test_safe_return_vals() public {
        Vat(bank).frob(gilk, address(this), abi.encodePacked(stack), int256(stack));
        (Vat.Spot spot, uint256 deal, uint256 tot) = Vat(bank).safe(gilk, self);
        ...
        uint256 tot1 = stack * uint256(val);
        assertEq(tot, tot1);
        ...
        rico_mint(WAD, true);
        Vat(bank).frob(gilk, address(this), "", -int256(Vat(bank).urns(gilk, self)));
        (spot, deal, tot) = Vat(bank).safe(gilk, self);
        assertTrue(spot == Vat.Spot.Safe);
    }
    ...
    function test_frob_refloat() public {
        ...
        // add ink to refloat
        Vat(bank).frob(gilk, address(this), abi.encodePacked(stack), int256(0));
        (spot,,) = Vat(bank).safe(gilk, self);
        assertTrue(spot == Vat.Spot.Safe);
    }
    ...
    function test_increasing_risk_sunk_urn() public {
        ...
        Vat(bank).frob(gilk, address(this), abi.encodePacked(int256(0)), int256(-1));
        Vat(bank).frob(gilk, address(this), abi.encodePacked(int256(1)), int256(0));

        // should not be able to decrease ink or increase art of sunk urn
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, address(this), abi.encodePacked(int256(10)), int256(1));
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, address(this), abi.encodePacked(int256(-1)), int256(-1));
    }
    ...
    function test_bail_hook() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD));
        uint256 vowgoldbefore = gold.balanceOf(bank);
    ...
    function test_bail_price() public {
        ...
        assertEq(rico.balanceOf(address(guy)), uint256(0));
    ...
    function test_bail_price_pup_1() public {
        ...
        feedpush(grtag, bytes32(750 * RAY), type(uint256).max);
        uint256 expected = rmul(borrow, rmul(RAY * 3 / 4, rmash(RAY * 3 / 4, pep, pop, pup)));
        ...
        feedpush(grtag, bytes32(1000 * RAY), type(uint256).max);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(borrow));
        feedpush(grtag, bytes32(250 * RAY), type(uint256).max);
        assertEq(abi.decode(guy.bail(gilk, self), (uint256)), WAD);
    ...
    function test_bail_refund() public {
        ...
        Vat(bank).filh(gilk, "pep", empty, bytes32(uint256(2)));
        Vat(bank).filh(gilk, "pop", empty, bytes32(pop));
        uint256 borrow = WAD * 500;
        feedpush(grtag, bytes32(1000 * RAY), type(uint256).max);
        // frob to edge of safety
        Vat(bank).frob(gilk, self, abi.encodePacked(dink), int256(borrow));
        ...
        uint256 expected_full = rmul(wmul(borrow * 2, dink * 75 ** 3 / 100 ** 3), pop);
        uint256 guy_earn = wmul(dink, wdiv(borrow, expected_full));
        assertEq(gold.balanceOf(address(guy)), guy_earn);

        // excess ink should be sent back to urn holder, not left in urn
        uint256 ink_left = _ink(gilk, self);
    ...
    function test_rico_join_exit() public _chap_ {
        ...
        uint256 self_gold_bal0 = gold.balanceOf(self);
        uint256 self_rico_bal0 = rico.balanceOf(self);
        ...
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(10)), int256(11));

        // revert for trying to exit gems from other users
        vm.expectRevert(Math.ErrUintUnder.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(-1)), 0);

        // gems are taken from user when joining, and rico given to user
        Vat(bank).frob(gilk, self, abi.encodePacked(stack), int256(stack / 2));
        uint256 self_gold_bal1 = gold.balanceOf(self);
        uint256 self_rico_bal1 = rico.balanceOf(self);
        ...
        rico.mint(self, 1);
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(stack)), -int256(stack / 2));
        uint256 self_gold_bal2 = gold.balanceOf(self);
        uint256 self_rico_bal2 = rico.balanceOf(self);
    ...
    function test_simple_rico_flash_mint() public _chap_ {
        uint256 initial_rico_supply = rico.totalSupply();
    ...
    function test_repayment_failure() public _chap_ {
        ...
        uint256 chap_gold = gold.balanceOf(achap);
        uint256 chap_rico = rico.balanceOf(achap);
    ...
    function test_rico_wind_up_and_release() public _chap_ {
        uint256 lock = 300 * WAD;
        uint256 draw = 200 * WAD;

        uint256 flash_gold1 = gold.balanceOf(achap);
        uint256 flash_rico1 = rico.balanceOf(achap);
        uint256 hook_gold1 = gold.balanceOf(address(tokhook));
        uint256 hook_rico1 = rico.balanceOf(address(tokhook));
        ...
        uint256 ink = _ink(gilk, achap);
        uint256 art = _art(gilk, achap);
        assertEq(ink, lock);
        assertEq(art, draw);
    ...
    function test_rejects_unsafe_frob() public {
        uint256 ink = _ink(gilk, self);
        uint256 art = _art(gilk, self);
        assertEq(ink, 0);
        assertEq(art, 0);

        // no collateral...shouldn't be able to borrow
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(0)), int256(WAD));
    }

    // amount of rico owed to pay down the CDP
    function owed() internal returns (uint256) {
        // update rack first
        Vat(bank).drip(gilk);
    ...
    function test_rest_monotonic() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD + 1), int256(WAD + 1));
    ...
    function test_rest_drip_toggle_ones() public {
        ...
        rico_mint(1, true);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(1)), int256(1));
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(1)), -int256(1));
    ...
    function test_rest_drip_toggle_wads() public {
        ...
        uint256 art = _art(gilk, self);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(0)), -int256(art));
        assertEq(Vat(bank).rest(), RAY);
    ...
    function test_drip_neg_fee() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(100 * WAD), int256(50 * WAD));
        skip(1);
        uint256 pre_joy = Vat(bank).joy();
        Vat(bank).filk(gilk, "fee", bytes32(RAY));
        uint256 aft_joy = Vat(bank).joy();
        uint256 rake = aft_joy - pre_joy;
        assertGt(rake, 0);
    ...
    function test_feed_plot_safe() public {
        ...
        assertEq(uint256(safe0), uint256(Vat.Spot.Safe));

        Vat(bank).frob(gilk, self, abi.encodePacked(100 * WAD), int256(50 * WAD));

        (Vat.Spot safe1,,) = Vat(bank).safe(gilk, self);
        assertEq(uint256(safe1), uint256(Vat.Spot.Safe));
    ...
    function test_par() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(100 * WAD), int256(50 * WAD));
        (Vat.Spot spot,,) = Vat(bank).safe(gilk, self);
        assertEq(uint256(spot), uint256(Vat.Spot.Safe));
        ...
        assertEq(uint256(spot2), uint256(Vat.Spot.Sunk));
    }

    function test_frob_reentrancy_1() public {
        // frob works now
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(WAD)), int256(WAD));
    ...
    function test_frob_hook() public {
        ...
        uint256 goldbefore = gold.balanceOf(self);

        // frob should call frobhook
        bytes memory hookdata = abi.encodeCall(hook.frobhook, Hook.FHParams(self, gilk, self, abi.encodePacked(WAD), 0));
    ...
    function test_frob_hook_neg_dink() public {
        ...
        bytes memory hookdata =
            abi.encodeCall(hook.frobhook, Hook.FHParams(self, gilk, self, abi.encodePacked(-int256(WAD)), 0));
        vm.expectCall(address(hook), hookdata);
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(WAD)), 0);
    ...
    function test_bailhook_1() public {
        ...
        FrobHook hook = new FrobHook();
        Vat(bank).filk(gilk, "hook", bytes32(uint256(bytes32(bytes20(address(hook))))));
        feedpush(grtag, bytes32(RAY), type(uint256).max);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD));
        uint256 goldbefore = gold.balanceOf(self);

        // ZeroHook's safehook makes the urn unsafe
        ZeroHook zhook = new ZeroHook();
        Vat(bank).filk(gilk, "hook", bytes32(uint256(bytes32(bytes20(address(zhook))))));

        // check that bailhook called
        feedpush(grtag, bytes32(0), UINT256_MAX);
        bytes memory hookdata = abi.encodeCall(zhook.bailhook, Hook.BHParams(gilk, self, WAD, WAD, self, 0, 0));
    ...
    function test_frob_err_ordering_1() public {
        ...
        Vat(bank).frob(gilk, bank, abi.encodePacked(WAD), int256(WAD / 2));

        // right urn, should be unsafe
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD / 2 - 1));

        // safe, should be dusty
        feedpush(grtag, bytes32(RAY * 100), type(uint256).max);
        vm.expectRevert(Vat.ErrUrnDust.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD / 2 - 1));

        //non-dusty, should be ceily
        vm.expectRevert(Vat.ErrDebtCeil.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256((WAD + WAD / 1_000) / 2));

        // raising ceil should fix ceilyness
        File(bank).file("ceil", bytes32(WAD + WAD / 1_000));
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256((WAD + WAD / 10_000) / 2));
    }
    ...
    function test_frob_err_ordering_darts() public {
        ...
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(WAD * 2), int256(1 + WAD * RAY / Vat(bank).FEE_MAX()));
        ...
        vm.expectRevert(Vat.ErrUrnDust.selector);
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(WAD), -int256(1));
        ...
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(1));
    ...
    function test_frob_err_ordering_dinks_1() public {
        ...
        vm.startPrank(fsrc);
        gold.approve(bank, 1000 * WAD);
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(WAD * 2), int256(1 + WAD * RAY / FEE_2X_ANN));
        vm.stopPrank();
        ...
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(-int256(1)), int256(0));
        ...
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(int256(0)), int256(0));
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(int256(1)), int256(0));
    }
    ...
    function test_frob_err_ordering_dinks_darts() public {
        ...
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        Vat(bank).frob(gilk, fsrc, abi.encodePacked(-int256(1)), int256(1));
    ...
    function test_frob_ilk_uninitialized() public {
        ...
        Vat(bank).frob("hello", self, abi.encodePacked(WAD), int256(WAD));
    }
    ...
    function test_dtab_not_normalized() public {
        ...
        uint256 ricobefore = rico.balanceOf(self);
        Vat(bank).frob(gilk, self, abi.encodePacked(WAD), int256(WAD));
        uint256 ricoafter = rico.balanceOf(self);
    ...
    function test_frobhook_only_checks_dink() public {
        ...
        guy.frob(gilk, self, abi.encodePacked(int256(0)), int256(WAD));
    }
    ...
    function test_bail_drips() public {
        ...
        feedpush(grtag, bytes32(0), type(uint256).max);
        uint256 prevrack = Vat(bank).ilks(gilk).rack;
        Vat(bank).bail(gilk, self);
    ...
    function test_bail_return_value() public {
        ...
        assertEq(abi.decode(sold, (uint256)), WAD);
    }
    ...
    function test_ink_return_value() public {
        ...
        assertEq(abi.decode(ink, (uint256)), WAD);
    }
    ...
    function test_no_dink() public {
        ...
        Vat(bank).frob(gilk, self, "", -int256(WAD / 2));
    ...
    function test_ceil_big_rest() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(RAY), int256(RAY * 2 / 3));

        // make about 50 * RAY rest to exceed ceil while debt < ceil
        for (uint256 i = 0; i < 100; i++) {
            int256 dart = i % 2 == 0 ? -int256(RAY / 2) : int256(RAY / 2);
            Vat(bank).frob(gilk, self, "", dart);
        }

        // frob what space is left below ceil
        // ceiling check should take rest into account
        uint256 diff = Vat(bank).ceil() - Vat(bank).debt();
        uint256 rack = Vat(bank).ilks(gilk).rack;
        vm.expectRevert(Vat.ErrDebtCeil.selector);
        Vat(bank).frob(gilk, self, "", int256(rdiv(diff, rack)));
    ...
    function test_bail_pop_pep() public {
        ...
        uint256 pre_rico = rico.balanceOf(self);
        Vat(bank).bail(gilk, self);
        uint256 aft_rico = rico.balanceOf(self);
        uint256 paid = pre_rico - aft_rico;

        // liqr is 1.0 so 1/6 backed
        // Estimate amount paid, put in a wad of gold now priced at 1/6
        uint256 tot = WAD / 6;
        uint256 rush = 6;
        uint256 est = rmul(tot, pop) / rush ** pep;
    ...
    function test_deal_but_not_wild() public {
        ...
        Vat(bank).filh(gilk, "pep", empty, bytes32(uint256(4)));
    ...
    function test_frob_safer_over_ceilings() public {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(2000 * WAD), int256(1000 * WAD));

        // over line, under ceil
        Vat(bank).filk(gilk, "line", bytes32(0));

        // safer dart
        Vat(bank).frob(gilk, self, "", -int256(WAD));
    ...
    function test_bail_moves_line() public {
        // defensive line
        uint256 dink = WAD * 1000;
        uint256 borrow = WAD * 1000;
        uint256 line0 = RAD * 1000;
        ...
        Vat(bank).filh(gilk, "liqr", empty, bytes32(RAY));
        Vat(bank).filh(gilk, "pep", empty, bytes32(uint256(1)));
        ...
        feedpush(grtag, bytes32(RAY / 2), type(uint256).max);

        uint256 sr0 = rico.balanceOf(self);
        uint256 sg0 = gold.balanceOf(self);
        Vat(bank).bail(gilk, self);
        uint256 line1 = Vat(bank).ilks(gilk).line;
        uint256 sr1 = rico.balanceOf(self);
        uint256 sg1 = gold.balanceOf(self);
        ...
        vm.expectRevert(Vat.ErrDebtCeil.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(dink), int256(borrow / 4 + 1));
        // barely under line
        Vat(bank).frob(gilk, self, abi.encodePacked(dink), int256(borrow / 4));
        ...
        feedpush(grtag, bytes32(RAY / 10), type(uint256).max);
        Vat(bank).bail(gilk, self);

        // fees or line modifications can lead to loss > capacity, check no underflow
        uint256 line2 = Vat(bank).ilks(gilk).line;
        assertEq(line2, 0);
    }
    ...
    function test_dink_size() public {
        ...
        vm.expectRevert();
        Vat(bank).frob(gilk, self, abi.encodePacked(int8(0)), int256(0));
        vm.expectRevert();
        Vat(bank).frob(gilk, self, abi.encodePacked(int200(0)), int256(0));

        Vat(bank).frob(gilk, self, abi.encodePacked(int256(0)), int256(0));
        Vat(bank).frob(gilk, self, abi.encodePacked(""), int256(0));
    }
    ...
    function test_ErrTransfer() public {
        ...
        Vat(bank).filh(gilk, "gem", empty, bytes32(bytes20(address(fgem))));
        vm.expectRevert(ERC20Hook.ErrTransfer.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(WAD / 2)), 0);
    }
    ...
    function test_ErrHookCallerNotBank() public {
        ...
        bytes memory indata = abi.encodeWithSelector(Hook.safehook.selector, gilk, self);
...
contract BadDataHook {
    uint256 count = type(uint256).max / 2;

    function frobhook(Hook.FHParams calldata) external returns (uint256, uint256) {
        return (count++, type(uint256).max);
    }

    function safehook(bytes32, address) external pure returns (uint256, uint256) {
...
contract OnlyInkHook is Hook {
    function frobhook(FHParams calldata p) external payable returns (bool) {
        ...
        return int256(uint256(bytes32(p.dink[:32]))) >= 0;
    }
...
contract BailFrobReentrancyHook is Bank, Hook {
    ...
    function bailhook(BHParams calldata p) external payable returns (bytes memory) {
        ...
        Vat(address(this)).frob(p.i, p.u, "", int256(WAD));
        return abi.encodePacked("");
    }
