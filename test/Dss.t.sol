...
import {RicoSetUp, Guy, ERC20Hook, Vat, Vow, Gem, Ball, File, Bank} from "./RicoHelper.sol";
...
contract Usr is Guy {
    ...
    function can_frob(bytes32 ilk, address u, bytes calldata dink, int256 dart) public returns (bool) {
...
contract DssJsTest is Test, RicoSetUp {
    ...
    uint256 goldprice = 40 * RAY / 110;
    uint256 starting_gem = 10000 * WAD;
    ...
    function setUp() public {
        ...
        Vat(bank).filk(gilk, bytes32("fee"), bytes32(uint256(RAY)));
...
contract DssFrobTest is DssVatTest {
    ...
    function test_lock() public _frob_ {
        ...
        // remove the ink
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(6 * WAD)), 0);
        assertEq(_ink(gilk, self), 0);
        assertEq(gold.balanceOf(self), 1000 * WAD);
    }
    ...
    function test_calm() public _frob_ {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(10 * WAD), int256(9 * WAD));

        // only if under debt ceiling
        vm.expectRevert(Vat.ErrDebtCeil.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(0)), int256(2 * WAD));
    ...
    function test_cool() public _frob_ {
        ...
        Vat(bank).filk(gilk, "line", bytes32(10 * RAD));
        Vat(bank).frob(gilk, self, abi.encodePacked(10 * WAD), int256(8 * WAD));
    ...
    function test_safe() public _frob_ {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(10 * WAD), int256(5 * WAD));
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(0)), int256(6 * WAD));
    }
    ...
    function test_nice() public _frob_ {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(10 * WAD), int256(10 * WAD));
        ...
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(2 * WAD)), -int256(4 * WAD));

        // debt can't increase, even if ink increases more
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(5 * WAD), int256(WAD));

        // ink can decrease if end state is safe
        Vat(bank).frob(gilk, self, abi.encodePacked(-int256(WAD)), -int256(4 * WAD));
    ...
    function test_alt_callers() public _frob_ {
        ...
        ali.frob(gilk, a, abi.encodePacked(10 * WAD), int256(5 * WAD));
        ...
        assertTrue(ali.can_frob(gilk, a, abi.encodePacked(-int256(WAD)), 0));
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        bob.frob(gilk, a, abi.encodePacked(-int256(WAD)), 0);
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        cat.frob(gilk, a, abi.encodePacked(-int256(WAD)), 0);

        // the lad can free to anywhere - ***N/A no v or w***

        // only the lad can draw
        assertTrue(ali.can_frob(gilk, a, abi.encodePacked(int256(0)), int256(WAD)));
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        bob.frob(gilk, a, "", int256(WAD));
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        cat.frob(gilk, a, "", int256(WAD));
        ...
        // anyone can wipe
        assertTrue(ali.can_frob(gilk, a, "", -int256(WAD)));
        assertTrue(bob.can_frob(gilk, a, "", -int256(WAD)));
        assertTrue(cat.can_frob(gilk, a, "", -int256(WAD)));
    ...
    function test_hope() public _frob_ {
        ...
        assertTrue(ali.can_frob(gilk, a, abi.encodePacked(int256(0)), int256(WAD)));
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        bob.frob(gilk, a, abi.encodePacked(int256(0)), int256(WAD));
        vm.expectRevert(Bank.ErrWrongUrn.selector);
        cat.frob(gilk, a, abi.encodePacked(int256(0)), int256(WAD));
    ...
    function test_dust() public _frob_ {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(9 * WAD), int256(WAD));
        Vat(bank).filk(gilk, "dust", bytes32(5 * RAD));

        // draw to dusty amount
        vm.expectRevert(Vat.ErrUrnDust.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(5 * WAD), int256(2 * WAD));
        Vat(bank).frob(gilk, self, abi.encodePacked(int256(0)), int256(5 * WAD));
...
contract DssBiteTest is DssVatTest {
    ...
    function vow_Awe() internal view returns (uint256) {
        return Vat(bank).sin();
    }

    // vow_Woe N/A - no debt queue in vow

    function _surp() public view returns (int256) {
        int256 joy = int256(Vat(bank).joy());
        int256 sin = int256(Vat(bank).sin() / RAY);
        return joy - sin;
    }
    ...
    function test_happy_bite() public _bite_ {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(40 * WAD), int256(100 * WAD));
    ...
    function testFail_vault_is_safe() public _bite_ {
        feedpush(grtag, bytes32(RAY * 5 / 2), block.timestamp + 1000);
        Vat(bank).frob(gilk, self, abi.encodePacked(100 * WAD), int256(150 * WAD));
    ...
    function test_floppy_bite() public _bite_ {
        feedpush(grtag, bytes32(RAY * 5 / 2), block.timestamp + 1000);
        uint256 ricoamt = 100 * WAD;
        Vat(bank).frob(gilk, self, abi.encodePacked(40 * WAD), int256(ricoamt));
...
contract DssFoldTest is DssVatTest {
    ...
    function draw(bytes32 ilk, uint256 amt) internal {
        gold.mint(self, amt);
        Vat(bank).drip(gilk);
        Vat(bank).frob(ilk, self, abi.encodePacked(WAD), int256(amt));
    }

    function tab(bytes32 ilk, address _urn) internal view returns (uint256) {
        uint256 art = _art(ilk, _urn);
        uint256 rack = Vat(bank).ilks(ilk).rack;
        return art * rack;
    }

    function test_fold() public _fold_ {
        uint256 fee = Vat(bank).ilks(gilk).fee;
        assertEq(fee, RAY);
        draw(gilk, WAD);
        ...
        skip(668226);
        uint256 mejoy0 = Vat(bank).joy() * RAY; // rad
        Vat(bank).drip(gilk);
        uint256 djoy = Vat(bank).joy() * RAY - mejoy0;
        uint256 tol = RAD / 1000;

        uint256 actual = RAD * 21 / 20;
...
contract DssClipTest is DssJsTest {
    ...
    function test_kick_zero_lot() public _clip_ {
        ...
        Vat(bank).frob(gilk, self, "", -int256(Vat(bank).urns(gilk, self)));
...
contract DssVowTest is DssJsTest {
    ...
    function test_no_reflop() public _vow_ {
        ...
        Vow(bank).keep(single(gilk));
        uint256 sr2 = rico.balanceOf(self);
    ...
    function test_flap() public _vow_ {
        ...
        Vat(bank).frob(gilk, self, abi.encodePacked(200 * WAD), int256(100 * WAD));
    ...
    function test_no_surplus_after_good_flop() public _vow_ {
        ...
        uint256 self_rico1 = rico.balanceOf(self);
        Vow(bank).keep(empty);
        uint256 self_rico2 = rico.balanceOf(self);

        // should have lost some rico to risk sale +1 extra for rounding
        uint256 banks_expected_rico = self_rico1 - self_rico2 + 1;
...
contract DssDogTest is DssJsTest {
    ...
    function setUrn(uint256 ink, uint256 art) internal {
        (bytes32 price, uint256 ttl) = feedpull(grtag);
        feedpush(grtag, bytes32(2 * RAY * art / ink), UINT256_MAX);
        Vat(bank).frob(gilk, self, abi.encodePacked(ink), int256(art));
        feedpush(grtag, price, ttl);
    }

    function test_bark_basic() public _dog_ {
        uint256 init_ink = WAD;
    ...
    function test_bark_dusty_vault() public {
        ...
        vm.expectRevert(Vat.ErrUrnDust.selector);
        Vat(bank).frob(gilk, self, abi.encodePacked(200000 * WAD), int256(199 * WAD));
    }
