...
contract BallTest is BaseHelper {
    ...
    uint256 initial_risk_supply = 1000000 * WAD;
    uint256 init_dust = 90 * RAD / 2000;
    uint256 start_time;
    ...
    function advance_chainlink() internal {
        ...
        uint256 skipped = block.timestamp + 100_000 - start_time;
    ...
    function setUp() public {
        ...
        Gem(WETH).approve(bank, type(uint256).max);
        ...
        safedart = int256(wethamt * uint256(val) / init_par * 10 / 11);
    }
    ...
    modifier _flap_after_() {
        ...
        uint256 pre_bank_risk = Gem(risk).balanceOf(bank);
        uint256 pre_bank_rico = Gem(rico).balanceOf(bank);
        uint256 pre_bank_joy = Vat(bank).joy();
        uint256 pre_user_risk = Gem(risk).balanceOf(self);
        uint256 pre_user_rico = Gem(rico).balanceOf(self);
        uint256 pre_risk_sup = Gem(risk).totalSupply();

        vm.expectCall(rico, abi.encodePacked(Gem.mint.selector));
        vm.expectCall(risk, abi.encodePacked(Gem.burn.selector));
        Vow(bank).keep(ilks);

        uint256 aft_bank_risk = Gem(risk).balanceOf(bank);
        uint256 aft_bank_rico = Gem(rico).balanceOf(bank);
        uint256 aft_bank_joy = Vat(bank).joy();
        uint256 aft_user_risk = Gem(risk).balanceOf(self);
        uint256 aft_user_rico = Gem(rico).balanceOf(self);
        uint256 aft_risk_sup = Gem(risk).totalSupply();
    ...
    modifier _balanced_after_() {
        ...
        uint256 me_risk_1 = Gem(risk).balanceOf(self);
        uint256 me_rico_1 = Gem(rico).balanceOf(self);

        Vow(bank).keep(ilks);

        uint256 me_risk_2 = Gem(risk).balanceOf(self);
        uint256 me_rico_2 = Gem(rico).balanceOf(self);
    ...
    function test_basic_feeds() public {
        ...
        uint256 vox_price = uint256(val);
        assertGt(uint256(vox_price), init_par * 99 / 100);
        assertLt(uint256(vox_price), init_par * 100 / 99);

        // should have a reasonable weth:ref (==weth:gold) price
        (val,) = fb.pull(address(divider), WETH_REF_TAG);
        assertClose(uint256(val), 1554 * RAY / 1925, 100);
    }
    ...
    function test_eth_denominated_ilks_feed() public {
        ...
        assertClose(rdiv(uint256(rai_ref_price), uint256(weth_ref_price)), uint256(rai_weth_price), 1000);
    }
    ...
    function test_ball_1() public {
        ...
        Vat(bank).frob(wilk, self, abi.encodePacked(int256(0)), safedart);
    }
    ...
    function test_fee_bail_flop() public _flop_after_ {
        ...
        uint256 meweth = WethLike(WETH).balanceOf(self);
    ...
    function test_ball_pay_flap_success() public _balanced_after_ {
        ...
        uint256 art_pre = Vat(bank).urns(wilk, self);
        uint256 ink_pre = _ink(wilk, self);

        assertEq(Vow(bank).ramp().wel, RAY);
        vm.expectCall(rico, abi.encodePacked(Gem.mint.selector));
        Vow(bank).keep(ilks); // drips

        uint256 rack = Vat(bank).ilks(wilk).rack;
        uint256 dust = Vat(bank).ilks(wilk).dust;
        int256 dart = -int256((art_pre * rack - dust) / rack);

        Vat(bank).frob(wilk, self, abi.encodePacked(int256(0)), dart);

        uint256 art_aft = Vat(bank).urns(wilk, self);
        uint256 ink_aft = _ink(wilk, self);
    ...
    function test_bounds_fee() public {
        ...
        uint256 fee_max = Vat(bank).FEE_MAX();
    ...
    function test_bounds_rel() public {
        ...
        Vat(bank).frob(wilk, self, abi.encodePacked(int256(WAD)), int256(WAD));

        // prank a low but nonzero risk:rico price so no reflop error
        vm.startPrank(fsrc);
        fb.push(RISK_RICO_TAG, bytes32(uint256(1)), UINT256_MAX);
        fb.push(WETH_REF_TAG, bytes32(uint256(0)), UINT256_MAX);
        vm.stopPrank();

        // create some sin
        Vat(bank).bail(wilk, self);

        uint256 supply_pre = Gem(risk).totalSupply();
        Vow(bank).keep(empty);
    ...
    function test_bounds_2() public {
        ...
        File(bank).file("how", bytes32(RAY));
        uint256 cap_max = File(bank).CAP_MAX();
        ...
        File(bank).file("how", bytes32(uint256(1000000000000003652500000000)));
        vm.expectRevert(Bank.ErrBound.selector);
