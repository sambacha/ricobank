...
contract ERC20Handler is Test, Local, RicoSetUp {
    ...
    bytes32[] public ilks;
    mapping(address actor => int256 offset) public ink_offset; // track bailed inter-actor weth
    ...
    constructor() {
        ...
        for (uint256 i = 1; i < NUM_ACTORS + 1; ++i) {
        ...
        minPar = par;
        rico_ref_val = par;
        artCap = int256(ACTOR_WETH * weth_ref_val / par);
    ...
    function frob(uint256 actorSeed, uint256 urnSeed, int256 ink, int256 art) public _larp_(actorSeed) {
        ink = bound(ink, -int256(ACTOR_WETH), int256(ACTOR_WETH));
    ...
    function bail(uint256 actorSeed, uint256 urnSeed) public _larp_(actorSeed) {
        ...
        uint256 pre_weth = Gem(WETH).balanceOf(currentActor);

        address urn = actors[bound(urnSeed, 0, actors.length - 1)];
        Vat(bank).bail(WETH_ILK, urn);

        uint256 aft_weth = Gem(WETH).balanceOf(currentActor);
        int256 sold = int256(aft_weth) - int256(pre_weth);
    ...
    function date(uint64 _ent) public _self_ {
        ...
        uint256 ent = uint256(_ent);
        uint256 stale_idx = type(uint256).max;
        if (ent * 100 / 99 > type(uint64).max) stale_idx = ent % tags.length;
        for (uint256 i; i < tags.length; i++) {
            (bytes32 val,) = feedpull(tags[i]);
            uint256 ttl = i == stale_idx ? 0 : block.timestamp * 2;
    ...
    function norev_frob(uint256 actorSeed, uint256 urnSeed, int256 ink, int256 art) public _larp_(actorSeed) {
        ...
        uint256 pre_ink = abi.decode(Vat(bank).ink(WETH_ILK, urn), (uint256));
        uint256 aft_ink = pre_ink;
        if (ink < 0) {
            if (uint256(-ink) > pre_ink) return;
            aft_ink -= uint256(-ink);
        } else if (ink > 0) {
            if (uint256(ink) > Gem(WETH).balanceOf(currentActor)) return;
            aft_ink += uint256(ink);
        }
        ...
        uint256 liqr = uint256(Vat(bank).geth(WETH_ILK, "liqr", empty));

        if (art < 0) {
            // avoid underflow
            if (uint256(-art) > pre_art) return;
            // need to own more than amount to burn
            if ((rmul(rack, uint256(-art)) + 1) > rico.balanceOf(currentActor)) return;
            // don't leave dust
            uint256 aft_art = pre_art - uint256(-art);
            ...
            uint256 pre_tart = Vat(bank).ilks(WETH_ILK).tart;
            if (add(pre_tart, art) * rack > Vat(bank).ilks(WETH_ILK).line) return;
            if ((Vat(bank).debt() + rmul(rack, uint256(art)) + Vat(bank).rest() / RAY) > Vat(bank).ceil()) return;
        } else {
    ...
    function norev_mark(bool up) public _self_ {
        rico_ref_val = up ? rico_ref_val * 101 / 100 : rico_ref_val * 100 / 101;
        feed.push(RICO_REF_TAG, bytes32(rico_ref_val), type(uint256).max);
    }

    function norev_move(bool up) public _self_ {
        weth_ref_val = up ? weth_ref_val * 5 / 4 : weth_ref_val * 4 / 5;
        weth_ref_max = max(weth_ref_max, weth_ref_val);
        feedpush(WETH_REF_TAG, bytes32(weth_ref_val), type(uint256).max);
    }
    ...
    function norev_bail(uint256 actorSeed, uint256 urnSeed) public _larp_(actorSeed) {
        ...
        uint256 liqr = uint256(Vat(bank).geth(WETH_ILK, "liqr", empty));
        uint256 art = Vat(bank).urns(WETH_ILK, urn);
        uint256 ink = abi.decode(Vat(bank).ink(WETH_ILK, urn), (uint256));
    ...
    function fresh() public _self_ {
        ...
        for (uint256 i; i < tags.length; ++i) {
            (bytes32 val,) = feedpull(tags[i]);
            feedpush(tags[i], val, type(uint256).max);
        }
    }
}
