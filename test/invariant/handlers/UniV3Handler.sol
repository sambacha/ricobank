...
contract UniV3Handler is Test, Local, RicoSetUp {
    ...
    uint256 internal constant FREE = uint256(-int256(1));
    ...
    constructor() {
        ...
        feedpush(wrtag, bytes32(WETH_REF_VAL), type(uint256).max);
        ...
        feedpush(grtag, bytes32(GOLD_REF_VAL), type(uint256).max);
        ...
        uint256 gold0 = gold.balanceOf(self);
        uint256 weth0 = weth.balanceOf(self);

        Vat(bank).filk(uilk, "line", bytes32(100000 * RAD));

        weth.approve(address(nfpm), type(uint256).max);
        gold.approve(address(nfpm), type(uint256).max);
        ...
        for (uint256 i = 1; i < NUM_ACTORS + 1; ++i) {
            ...
            PoolArgs memory args =
                PoolArgs(Asset(agold, LP_AMOUNT), Asset(WETH, LP_AMOUNT), 500, X96, X96 * 3 / 4, X96 * 4 / 3, 10, actor);
            uint256 nft_id;
            for (uint256 j = 0; j < HOOK_ROOM + 1; ++j) {
        ...
        minPar = par;
        rico_ref_val = par;
        artCap = int256(LP_AMOUNT * (weth_ref_val + gold_ref_val) / par);
        artCap = artCap * 4 / 3;

        uint256 dist_gold = gold0 - gold.balanceOf(self);
        uint256 dist_weth = weth0 - weth.balanceOf(self);
        uint256 line = dist_gold * gold_ref_val + dist_weth * weth_ref_val;
        line = line * 4 / 3;
    ...
    function frob(uint256 actorSeed, uint256 urnSeed, uint256 tok, bool dir, int256 art) public _larp_(actorSeed) {
        ...
        uint256 _dir = dir ? LOCK : FREE;
        uint256 idx = bound(tok, 0, positions.length - 1);
        uint256 tokid = positions[idx];
