...
import {RicoSetUp, Guy, WethLike, Ball, Gem, Vat, Vow, File, Bank, Hook} from "./RicoHelper.sol";
...
contract NFTHookTest is Test, RicoSetUp {
    ...
    uint256 stack = WAD * 10;
    bytes32[] ilks;
    address[] gems;
    uint256[] wads;
    Flasher chap;
    address achap;
    uint256 goldwethtokid;
    uint256 golddaitokid;
    ...
    function setUp() public {
        ...
        PoolArgs memory args =
            PoolArgs(Asset(agold, 1000 * WAD), Asset(WETH, 1000 * WAD), 500, X96, X96 * 3 / 4, X96 * 4 / 3, 10, self);
        (goldwethtokid,,,) = create_and_join_pool(args);

        args = PoolArgs(Asset(agold, 1000 * WAD), Asset(DAI, 1000 * WAD), 500, X96, X96 * 3 / 4, X96 * 4 / 3, 10, self);
        ...
        feedpush(wrtag, bytes32(1000 * RAY), type(uint256).max);
        ...
        feedpush(grtag, bytes32(1900 * RAY), type(uint256).max);

        feedpush(drtag, bytes32(RAY), type(uint256).max);
    ...
    function test_nft_frob() public {
        ...
        uint256[] memory dink = new uint256[](2);
        (dink[0], dink[1]) = (1, goldwethtokid);
        uint256 ricobefore = rico.balanceOf(self);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(WAD));
    ...
    function test_max_urn_nfts() public {
        uint256 nft_id;
        PoolArgs memory args =
            PoolArgs(Asset(agold, 1 * WAD), Asset(WETH, 1 * WAD), 500, X96, X96 * 3 / 4, X96 * 4 / 3, 10, self);

        for (uint256 i = 0; i < HOOK_ROOM + 1; i++) {
    ...
    function test_nft_bail() public {
        ...
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(0 * WAD));

        // LP has a thousand of each, both worth 1000, liqr 1.0, max debt would be nearly 2M
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(uilk, self, "", int256((2_000_001) * WAD));
        Vat(bank).frob(uilk, self, "", int256((2_000_001 - 2) * WAD));
        // pay off to reset
        Vat(bank).frob(uilk, self, "", -int256(Vat(bank).urns(uilk, self)));
        ...
        feedpush(grtag, bytes32(1100 * RAY), type(uint256).max);
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(uilk, self, "", int256(2_099_000 * WAD));
        // remains in same tick but collateral is is worth more
        Vat(bank).frob(uilk, self, "", int256(2_090_000 * WAD));
        Vat(bank).frob(uilk, self, "", -int256(Vat(bank).urns(uilk, self)));
        ...
        feedpush(grtag, bytes32(1_000_000_000_000 * RAY), type(uint256).max);
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(uilk, self, "", int256(2_400_300 * WAD));
        Vat(bank).frob(uilk, self, "", int256(2_300_000 * WAD));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        Gem(agold).transfer(pool, uint256(amount0Delta));
    }
    ...
    function test_moving_tick_with_swap() public {
        ...
        feedpush(grtag, bytes32(1000 * RAY), type(uint256).max);
        // frob about limit
        Vat(bank).frob(uilk, self, abi.encode(dink), int256((2_000_000 - 1) * WAD));

        // swap to push tick far from oracle expected value
        address pool = IUniswapV3Factory(factory).getPool(agold, WETH, 500);
        (uint160 sqrtPriceX96,,,,,,) = IUniswapV3Pool(pool).slot0();
        IUniswapV3Pool(pool).swap(address(this), true, int256(100 * WAD), sqrtPriceX96 / 1000, abi.encode(pool));
        ...
        Vat(bank).frob(uilk, self, "", int256(50 * WAD));
        // that was limit, one more won't work
        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(uilk, self, "", int256(1 * WAD));
    }
    ...
    function test_nft_bail_price() public {
        ...
        uint256[] memory dink = new uint256[](2);
        (dink[0], dink[1]) = (1, goldwethtokid);
        uint256 borrow = WAD * uint256(2_000_000 - 1);
        assertEq(nfpm.ownerOf(goldwethtokid), self);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(borrow));
        assertEq(nfpm.ownerOf(goldwethtokid), bank);

        // set prices to 75%
        feedpush(wrtag, bytes32(750 * RAY), type(uint256).max);
    ...
    function test_nft_bail_refund() public {
        ...
        uint256[] memory dink = new uint256[](2);
        (dink[0], dink[1]) = (1, goldwethtokid);
        uint256 borrow = WAD * uint256(1_000_000 - 1);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(borrow));
        ...
        uint256 expected_cost_for_keeper = rmul(wmul(borrow * 2, WAD * 75 ** 3 / 100 ** 3), pop);
        ...
        uint256 self_pre_bail_rico = rico.balanceOf(self);
        guy.bail(uilk, self);
        ...
        uint256 refund = rico.balanceOf(self) - self_pre_bail_rico;
        uint256 expected_refund = expected_cost_for_keeper - borrow;
    ...
    function test_multipos() public {
        ...
        feedpush(wrtag, bytes32(0 * RAY), type(uint256).max);
        vm.expectRevert(Vat.ErrSafeBail.selector);
        Vat(bank).bail(uilk, self);

        feedpush(drtag, bytes32(RAY / uint256(200_000)), type(uint256).max);
        feedpush(grtag, bytes32(RAY / uint256(200_000)), type(uint256).max);
        (, uint256 deal,) = Vat(bank).safe(uilk, self);

        // only LP of value is golddai worth 1% -> discount should be 0.01 ^ 2
        uint256 wad_cost = rmul(WAD, rmul(deal, rmul(deal, deal)));
        rico_mint(WAD, true);
        rico.transfer(address(guy), wad_cost);

        uint256 guy_rico_before = rico.balanceOf(address(guy));
        guy.bail(uilk, self);
        uint256 guy_rico_after = rico.balanceOf(address(guy));
        uint256 bail_price = guy_rico_before - guy_rico_after;
    ...
    function test_nft_frob_down() public {
        ...
        // remove golddaitokid
        dink = new uint256[](2);
        (dink[0], dink[1]) = (uint256(-int256(1)), golddaitokid);
        ...
        (dink[0], dink[1]) = (uint256(-int256(1)), goldwethtokid);
        Vat(bank).frob(uilk, self, abi.encode(dink), 0);
        assertEq(nfpm.ownerOf(goldwethtokid), self);
        assertEq(nfpm.ownerOf(golddaitokid), bank);
        (spot,,) = Vat(bank).safe(uilk, self);
        ...
        // remove both
        dink = new uint256[](3);
        (dink[0], dink[1], dink[2]) = (uint256(-int256(1)), golddaitokid, goldwethtokid);
        rico_mint(100, true); // rounding
        Vat(bank).frob(uilk, self, abi.encode(dink), -int256(WAD));
        assertEq(nfpm.ownerOf(goldwethtokid), self);
        assertEq(nfpm.ownerOf(golddaitokid), self);
        assertEq(abi.decode(Vat(bank).ink(uilk, self), (uint256[])).length, 0);
    }

    function test_nft_make_unsafe_by_rack() public {
        feedpush(drtag, bytes32(RAY), type(uint256).max);
        feedpush(grtag, bytes32(RAY), type(uint256).max);
        feedpush(wrtag, bytes32(RAY), type(uint256).max);

        // add goldwethtokid and golddaitokid at once
        uint256[] memory dink = new uint256[](3);
        (dink[0], dink[1], dink[2]) = (1, goldwethtokid, golddaitokid);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(900 * WAD));

        // if either token in a position has zero price feed position is worthless
        feedpush(drtag, bytes32(0), type(uint256).max);
        (Vat.Spot spot,,) = Vat(bank).safe(uilk, self);
        ...
        (, uint256 deal,) = Vat(bank).safe(uilk, self);
        uint256 wad_cost = rmul(rpow(deal, 2), 1000 * WAD);
        rico_mint(wad_cost, true);
        uint256[] memory tok_ids = abi.decode(Vat(bank).bail(uilk, self), (uint256[]));
    ...
    function test_frob_down_five() public {
        ...
        uint256[4] memory goldwethtokids;
        ...
        uint256[] memory dink = new uint256[](6);
        (dink[0], dink[1], dink[2], dink[3], dink[4], dink[5]) =
            (1, golddaitokid, goldwethtokids[0], goldwethtokids[1], goldwethtokids[2], goldwethtokids[3]);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(WAD));
        ...
        // only remove a few
        dink = new uint256[](4);
        (dink[0], dink[1], dink[2], dink[3]) = (uint256(-int256(1)), golddaitokid, goldwethtokids[1], goldwethtokids[3]);
        Vat(bank).frob(uilk, self, abi.encode(dink), -int256(WAD));
    ...
    function test_geth() public {
        assertEq(uint256(Vat(bank).geth(uilk, "room", empty)), HOOK_ROOM);
    ...
    function test_filh() public {
        ...
        // wrong xs length
        vm.expectRevert(Bank.ErrWrongKey.selector);
        Vat(bank).filh(uilk, "room", new bytes32[](1), bytes32(uint256(6)));

        Vat(bank).filh(uilk, "room", empty, bytes32(uint256(5)));
        Vat(bank).filh(uilk, "wrap", empty, bytes32(bytes20(fsrc)));

        // wrong key
        vm.expectRevert(Bank.ErrWrongKey.selector);
        Vat(bank).filh(uilk, "blah", single(bytes32(bytes20(fsrc))), bytes32(uint256(5)));

        // wrong xs length
        vm.expectRevert(Bank.ErrWrongKey.selector);
        Vat(bank).filh(uilk, "src", empty, bytes32(uint256(5)));
        vm.expectRevert(Bank.ErrWrongKey.selector);
        Vat(bank).filh(uilk, "src", new bytes32[](2), bytes32(uint256(5)));
        vm.expectRevert(Bank.ErrWrongKey.selector);
        Vat(bank).filh(uilk, "liqr", empty, bytes32(uint256(5)));

        Vat(bank).filh(uilk, "src", single(bytes32(bytes20(fsrc))), bytes32(uint256(10)));
        Vat(bank).filh(uilk, "tag", single(bytes32(bytes20(fsrc))), bytes32(uint256(100)));
        Vat(bank).filh(uilk, "liqr", single(bytes32(bytes20(fsrc))), bytes32(uint256(100 * RAY)));
    }
    ...
    function test_not_found() public {
        ...
        Vat(bank).frob(uilk, self, abi.encode(dink), -int256(WAD / 2));
    ...
    function test_no_feed() public {
        ...
        Vat(bank).filh(uilk, "src", single(bytes32(bytes20(WETH))), bytes32(uint256(0)));
        Vat(bank).filh(uilk, "src", single(bytes32(bytes20(agold))), bytes32(bytes20(fsrc)));
        ...
        Vat(bank).filh(uilk, "src", single(bytes32(bytes20(agold))), bytes32(uint256(0)));
        vm.expectRevert();
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(WAD));
    ...
    function test_bail_uni_moves_line() public {
        uint256 borrow = WAD * 2000 - 10;
        uint256 line0 = RAD * 2000;
        uint256 liqr = RAY * 1;

        Vat(bank).filk(uilk, "line", bytes32(line0));
        Vat(bank).filh(uilk, "liqr", single(bytes32(bytes20(WETH))), bytes32(liqr));
        Vat(bank).filh(uilk, "liqr", single(bytes32(bytes20(agold))), bytes32(liqr));
        Vat(bank).filh(uilk, "pep", empty, bytes32(uint256(1)));
        ...
        Vat(bank).bail(uilk, self);
        uint256 line1 = Vat(bank).ilks(uilk).line;
        ...
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(borrow) / 3);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(borrow) / 4);
        ...
        uint256 line2 = Vat(bank).ilks(uilk).line;
        assertEq(line2, 0);
    }
    ...
    function test_uni_errors_1() public {
        ...
        assertTrue(unihook.frobhook(Hook.FHParams(self, uilk, agold, "", -int256(1))));

        // lock ink for someone else
        uint256[] memory dink = new uint256[](1);
        dink[0] = 1;
        assertTrue(unihook.frobhook(Hook.FHParams(self, uilk, agold, abi.encode(dink), 0)));
        ...
        uint8[] memory dink8 = new uint8[](1);
        dink8[0] = uint8(-int8(1));
        vm.expectRevert(UniNFTHook.ErrDir.selector);
        unihook.frobhook(Hook.FHParams(self, uilk, agold, abi.encode(dink8), 0));

        // single uint40, similar to previous
        uint40[] memory dink40 = new uint40[](1);
        dink40[0] = uint40(-int40(1));
        vm.expectRevert(UniNFTHook.ErrDir.selector);
        ...
        unihook.file("room", uilk, empty, bytes32(uint256(1)));
    ...
    function setrico(address hook, address _rico) internal {
        ...
        // rico @idx 0
        vm.store(hook, bank_pos, bytes32(uint256(bytes32(bytes20(_rico))) >> (12 * 8)));
    }

    function setfb(address hook, address _fb) internal {
        // fb @idx 1
        bytes32 bank_info = "ricobank.0";
        bytes32 fb_pos = bytes32(uint256(keccak256(abi.encodePacked(bank_info))) + 1);
        vm.store(hook, fb_pos, bytes32(uint256(bytes32(bytes20(_fb))) >> (12 * 8)));
    }
    ...
    function test_empty_ink_bailhook() public {
        ...
        uint256[] memory ids = abi.decode(unihook.bailhook(p), (uint256[]));
    ...
    modifier _raw_() {
        setrico(address(unihook), address(rico));
        rico.ward(address(unihook), true);
        unihook.file("room", uilk, empty, bytes32(uint256(5)));
    ...
    function test_multi_safehook() public _raw_ {
        ...
        (,, uint256 ttl) = unihook.safehook(uilk, self);
    ...
    function test_ratio_oob() public _raw_ {
        ...
        uint256 MAX_SHIFT = 2 ** (256 - 96) - 1;
        assertLt(uint256(bytes32(bytes20(agold))), uint256(bytes32(bytes20(WETH))));

        // very high but not too high token1 price
        // uni hook scales feeds down to wad
        feedpush(grtag, bytes32(MAX_SHIFT * BLN), UINT256_MAX);
        feedpush(wrtag, bytes32(RAY), UINT256_MAX);
        (uint256 tot,,) = unihook.safehook(uilk, self);
        (, uint256 wamt) = IUniWrapper(uniwrapper).total(INFPM(UNI_NFT_ADDR), goldwethtokid, type(uint160).max);
    ...
    function test_rico_nft_frob() public {
        ...
        feedpush(grtag, bytes32(1 * RAY), type(uint256).max);
        Vat(bank).filh(uilk, "liqr", single(bytes32(bytes20(agold))), bytes32(11 * RAY / 10));
        rico_mint(200 * WAD, true);

        uint256 goldricotokid;

        PoolArgs memory args =
            PoolArgs(Asset(agold, 100 * WAD), Asset(arico, 100 * WAD), 500, X96, X96 * 3 / 4, X96 * 4 / 3, 10, self);
        ...
        uint256 borrow_limit = WAD * 200 * 10 / 11;
        uint256 offset = borrow_limit / 1_000_000;

        vm.expectRevert(Vat.ErrNotSafe.selector);
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(borrow_limit + offset));
        Vat(bank).frob(uilk, self, abi.encode(dink), int256(borrow_limit - offset));
        // reset debt to none
        Vat(bank).frob(uilk, self, "", -int256(borrow_limit - offset));
