...
import {RicoSetUp, Guy, BankDiamond, Bank, File, Vat, Vow, Hook, Gem, Ball} from "./RicoHelper.sol";
...
contract IntegrationTest is Test, RicoSetUp {
    ...
    function test_joy_accounting() public {
        ...
        feedpush(grtag, bytes32(RAY / 4), type(uint256).max);
    ...
    function test_flap_joy_direction() public _check_integrity_after_ {
        ...
        uint256 rico_sup0 = rico.totalSupply();
        uint256 risk_sup0 = risk.totalSupply();
        uint256 joy0 = Vat(bank).joy();

        // joy should decrease, because it's being flapped
        Vow(bank).keep(single(gilk));

        uint256 rico_sup1 = rico.totalSupply();
        uint256 risk_sup1 = risk.totalSupply();
        uint256 joy1 = Vat(bank).joy();
        assertLt(joy1, joy0);
